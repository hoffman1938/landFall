 ...
 *
 * v4 POSITIONING NOTE. This used to pin itself to `top-2` of the board, which
 * put it straight over the compact Storm Clock at `top-12` on every screen
 * below `lg` — the countdown, the one thing a player must be able to read at
 * all times, was covered for eleven seconds after joining a table. It no longer
 * positions itself at all: the shell owns a single notice band above the deck
 * (see `BoardNotices`), the band shows one notice at a time, and the clock is
 * never touched. The state lives in a hook so the band can decide priority
 * without having to guess whether this card is going to render.
 */
...
/** The intro's state, hoisted so the notice band can rank it against a toast. */
export function useTableIntro(): { intro: Intro | null; dismiss(): void } {
  ...
  return { intro, dismiss: () => setIntro(null) };
}

/** Presentational only — the band places it; it never places itself. */
export function TableIntroCard({ intro, onDismiss }: { intro: Intro; onDismiss(): void }) {
  return (
    <div
      role="status"
      aria-live="polite"
      className="lf-appear lf-overlay pointer-events-none relative w-full max-w-md rounded-lg px-4 py-3"
    >
      <button
        type="button"
        onClick={onDismiss}
        className="pointer-events-auto absolute right-1.5 top-1.5 flex h-11 w-11 items-center justify-center rounded-md text-[var(--lf-dim)] hover:text-[var(--lf-text)]"
      ...
      <ul className="mt-2 space-y-1.5 text-[12px] leading-snug text-[var(--lf-dim)]">
        <li className="flex gap-2">
          ...
          <span>
            <b className="text-[var(--lf-text)]">
              Your bet: {fmt(intro.minMinor)}–{fmt(intro.maxMinor)}
            </b>{' '}
        ...
        <li className="flex gap-2">
          ...
          <span>
            ...
            The deck shows the live number as <b className="text-[var(--lf-text)]">Max now</b>, and
            it rises as players join.
          </span>
        </li>
        <li className="flex gap-2">
          ...
          <span>
            This table has <b className="text-[var(--lf-text)]">its own jackpot</b>, built only from
            rounds played here. One safe player wins it on a jackpot round — the bigger your bet,
